                ...
                <div class="col-md-5">
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">Update Recorded Session Link</div>
                        </div>
                        <div class="card-body">
                           <form action="{{route('record.link.update', $record->id)}}" method="post">
                            @csrf
                            <div class="form-group">
                                <select class="form-control" name="course_id">
                                    <option value="">Select Course Title</option>
                                    @foreach($courses as $course)
                                    <option value="{{$course->id}}" {{$course->id == $record->course_id ? "selected" : " "}}>{{$course->title}}</option>
                                    @endforeach
                            ...
                            <div class="form-group">
                                <select required class="form-control" name="cohort_id">
                                    ...
                                    <option value="{{$cohort->id}}" {{$cohort->id == $record->cohort_id ? "selected": ""}}>{{$cohort->name}}</option>
                                    @endforeach
                           ...
                           <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="email2" value="{{$record->link}}" required name="link" placeholder="Enter Recorded Session Link">
